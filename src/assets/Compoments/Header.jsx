    ...
    return (
        <div className='w-full h-[200px] flex justify-between '>
                <div className=' ms-12 w-[40rem] flex justify-center items-center'>
                    <input type="text" placeholder="Shear for country" onChange={(e)=>{setshear(e.target.value)}} className={` ms-12 text-lg h-[4rem] input input-bordered w-full text-white ${darkMode?"text-white bg-[#2B3743]":"text-black bg-white"}`} />
                </div>
                <div className=' pe-14 w-[27rem] flex justify-center items-center'>
                    <div className="dropdown ">
                        <div tabIndex={0} role="button" className={`btn ps-5 m-1 h-[4rem] text-lg w-[16rem] hover:text-black  text-white ${darkMode?"text-white bg-[#2B3743]":"text-black bg-white"}`}>Filter by Region</div>
